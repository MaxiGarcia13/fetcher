...
- **Resume work later** — save requests in the sidebar; each snapshot stores the full request state encoded in the page URL.
- **Share or bookmark a request** — because method, URL, headers, params, and body live in query parameters, you can copy the address bar to reproduce the same setup.
...
```
src/
  components/     UI (request editor, panels, tabs, Monaco wrapper, saved sessions)
  domain/         HTTP and session types and helpers
  layouts/        App shell
  pages/          Index route and API routes
...
ISC — see [package.json](./package.json). d
